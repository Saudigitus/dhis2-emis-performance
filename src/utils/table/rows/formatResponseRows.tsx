import type { attributesProps } from "../../../types/api/WithRegistrationProps"
import type { dataValuesProps } from "../../../types/api/WithoutRegistrationProps"
import type {
...
export function formatResponseRows({
  ...
  //   setImmutableTeiData,
  programStage
}: FormatResponseRowsProps): RowsDataProps[] {
  ...
  for (const event of eventsInstances) {
    ...
    // setImmutableTeiData((prevState: any) => [
    //   ...prevState,
    //   {
    ...
    allRows.push({
      ...
      ...(marksDetails !== undefined
        ? { ...dataValues(marksDetails.dataValues, programStage) }
        : {}),
      ...attributes(teiDetails?.attributes ?? []),
      trackedEntity: event.trackedEntity,
      enrollment: event?.enrollment,
...
function dataValues(
  ...
  for (const dataElement of data) {
    let key = programStage
      ? dataElement.dataElement + "_" + programStage
