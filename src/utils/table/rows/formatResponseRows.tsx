...
export function formatResponseRows({ eventsInstances, teiInstances, marksInstances, programIndicatorsInstances, programStage, nextPstageEvents }: FormatResponseRowsProps): RowsDataProps[] {

    const allRows: RowsDataProps[] = []
    for (const event of eventsInstances) {
        const teiDetails = teiInstances.find(tei => tei.trackedEntity === event.trackedEntity)
        const registration = marksInstances.find(mark => (mark.trackedEntity === event.trackedEntity) && (mark?.enrollment === event?.enrollment))
        ...
        allRows.push({
            ...
            ...(registration !== undefined ? { ...dataValues(registration.dataValues, registration?.programStage) } : {}),
