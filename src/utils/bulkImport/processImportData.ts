import {fromPairs} from "./validateTemplae";
...
export const getMarksDataFromTemplate = (
    ...
    return objects.map(obj => {
        ...
        Object.keys(obj).forEach(key => {
            ...
            if (marksStages.some(match => key.includes(match))) {
...
function transformToDataValues(input: Record<string, any>): Array<{ dataElement: string, value: string }> {
    return Object.entries(input).flatMap(([key, value]) => {
        ...
        // Return an empty array if key doesn't match the pattern
        return [];
    });
}
