...
import type { ContentProps } from "../../../types/modal/ModalContentTypes"
...
import { useParams, usePostEvent } from "../../../hooks"
    ...
    if (
      ...
      for (const event of selected.selectedRows) {
        enrollments.push({
          ...
          status:
            allFields[0].assignedValue === "Dropout"
              ? "CANCELLED"
              : "COMPLETED",
          ...
          events: [
            {
              ...
              dataValues: [
                {
                  dataElement: allFields[0].id,
                  value: allFields[0].assignedValue
                }
              ]
            }
          ]
              ...
              <ModalActions>
                <ButtonStrip end>
                  {modalActions.map((action, i) => (
                    <Button key={i} {...action}>
                      {loadUpdateEvent && action.id === clickedButton ? (
