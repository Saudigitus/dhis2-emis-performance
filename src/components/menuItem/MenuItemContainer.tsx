...
function MenuItemContainer({ dataElementId, onToggle }: { dataElementId: string, onToggle: () => void }): React.ReactElement {
    ...
    const options = formatResponse(programConfigState, getDataStoreData?.registration?.programStage)?.find(element => element.id === dataElementId)?.options.optionSet.options ?? [];
