...
import {type PaginationState} from "../../types/bulkImport/Interfaces";
...
const SummaryDetails = (): React.ReactElement => {
    ...
    const total = students?.length ?? 0; // Total number of records for the current tab
        ...
        { (processingStage === 'template-processing') &&
            <>
                ...
                {total > 0 && <Pagination
                    ...
                    loading={false}
                    totalPerPage={displayData?.length ?? 0}
                />}
            </>
        }
        {
